...
var googleMapsV3 = (function() {
	...
	_.createMap = function(lat, lng, zoom, div, callback, failure) {
        ...
        if(google && google.maps) {
            gm = google.maps;
            gme = gm.event;
	        new _.map2D(div, lat, lng, zoom, callback, failure);
	    } else {
	    	if(failure) {
	...
	_.map2D = (function() {
		...
		function m(div, lat, lng, zoom, callback, failure) {
		...
		function w(map2D, baseUrl, version, layers, copyright, styles, srs, swLat, swLng, neLat, neLng, isBaseLayer) {
			...
			var layer = new gm.ImageMapType({
				getTileUrl: function (coord, zoom) {
					...
					var swConverted = srsConversion(swCoord);
					var neConverted = srsConversion(neCoord);
